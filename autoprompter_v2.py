...
def summarize_chunk(chunk):
    """텍스트 청크를 요약합니다."""
    messages = [
        {"role": "system", "content": "당신은 프롬프트 엔지니어링 전문가입니다. 주어진 텍스트를 요약할 때, 18개의 프롬프트 엔지니어링 기법을 명확히 식별하고 설명해야 합니다."},
        {"role": "user", "content": f"다음 프롬프트 엔지니어링 관련 텍스트를 요약하되, 각 기법을 명확히 식별하고 간단히 설명해주세요:\n\n{chunk}"}
    ]
...
def generate_system_prompt(summary):
    ...
    """
    messages = [
        {"role": "system", "content": "당신은 효과적인 시스템 프롬프트를 만드는 AI assistant입니다. 18개의 프롬프트 엔지니어링 기법을 적극적으로 활용해야 합니다."},
        {"role": "user", "content": f"이 프롬프트 엔지니어링 기법 요약을 바탕으로, 18개의 기법을 최대한 많이 활용하여 AI assistant가 다양한 작업을 더 잘 수행할 수 있도록 돕는 시스템 프롬프트를 만들어주세요. 프롬프트에는 반드시 한국어로 답변을 생성하라는 지시사항을 포함해야 합니다. 각 기법이 어떻게 적용되었는지 설명도 함께 제공해주세요:\n\n{summary}"}
    ]
...
def generate_diverse_prompt(summary, previous_prompts):
    ...
    messages = [
        ...
        {"role": "user", "content": f"이 프롬프트 엔지니어링 기법 요약을 바탕으로, 다음 이전 프롬프트와는 다른 새롭고 독특한 시스템 프롬프트를 만들어주세요. 프롬프트에는 반드시 한국어로 답변을 생성하라는 지시사항을 포함해야 합니다: {previous_prompts}\n\n요약:\n{summary}"}
    ]
    return create_chat_completion(messages)
...
def evolve_prompt(best_prompt, summary):
    ...
    messages = [
        ...
        {"role": "user", "content": f"이 시스템 프롬프트를 분석하고 프롬프트 엔지니어링 기법 요약을 바탕으로 개선된 버전의 프롬프트를 제공해주세요. 개선 사항에 대한 설명이 아닌, 실제로 사용할 수 있는 개선된 프롬프트 전체를 반환해야 합니다. 프롬프트에는 반드시 한국어로 답변을 생성하라는 지시사항을 포함해야 합니다.\n\n현재 프롬프트: {best_prompt}\n\n요약: {summary}"}
    ]
    return create_chat_completion(messages)
...
def evaluate_response_quality(response, task, prompt):
    """
    ...
    :param prompt: 사용된 시스템 프롬프트
    :return: 응답의 품질 점수 (1-10)와 코멘트
    """
    question, context = task
    messages = [
        {"role": "system", "content": "당신은 의료 QA 시스템의 응답과 사용된 프롬프트를 평가하는 전문가입니다. 다음 기준으로 1-10점 척도로 평가하세요: 1) 정확성 2) 관련성 3) 이해 용이성 4) 완전성 5) 안전성 6) 한국어 사용 7) 프롬프트 엔지니어링 기법 활용도"},
        {"role": "user", "content": f"다음 질문, 컨텍스트, 응답, 그리고 사용된 프롬프트를 평가해주세요.\n질문: {question}\n컨텍스트: {context}\n응답: {response}\n사용된 프롬프트: {prompt}\n평가는 'Rating: X/10' 형식으로 제공하고, 프롬프트 엔지니어링 기법의 활용도에 대해 간단히 코멘트해주세요."}
    ]
    ...
    return rating, content  # 점수와 코멘트를 함께 반환
...
def score_prompt(prompt, tasks):
    ...
    :return: 프롬프트의 총점과 코멘트 리스트
    """
    total_score = 0
    comments = []
    for task in tasks:
        result = evaluate_prompt(prompt, task)
        score, comment = evaluate_response_quality(result, task, prompt)
        total_score += score
        comments.append(comment)
    return total_score, comments

def analyze_prompt_techniques(prompt, summary):
    messages = [
        {"role": "system", "content": "당신은 프롬프트 엔지니어링 전문가입니다. 주어진 프롬프트에서 사용된 18개의 프롬프트 엔지니어링 기법을 식별하고 분석해야 합니다."},
        {"role": "user", "content": f"다음 프롬프트에서 사용된 프롬프트 엔지니어링 기법을 식별하고 분석해주세요. 각 기법이 어떻게 적용되었는지 설명해주세요.\n\n프롬프트: {prompt}\n\n요약된 18개 기법: {summary}"}
    ]
    return create_chat_completion(messages, max_tokens=500)
def auto_prompter(summary, tasks, iterations=5, initial_prompt=None):
    ...
    best_score, best_comments = score_prompt(best_prompt, tasks)
    ...
    for i in range(iterations):
        ...
        # 프롬프트 엔지니어링 기법 활용 분석
        technique_analysis = analyze_prompt_techniques(best_prompt, summary)
        print("프롬프트 엔지니어링 기법 활용 분석:")
...
if __name__ == "__main__":
    ...
    with open("prompt_guide.mdx", "r", encoding="utf-8") as file:
     ...
     # 원래의 시스템 프롬프트
    original_prompt =  """You are the world’s most authoritative health checkup AI ChatGPT in the question-answering task in the field of medicine and healthcare, providing answers with given context to non-medical professionals. \
                            The given context is an excerpt of data in html format.\

                            If you answer accurately, you will be paid incentives 1million USD  proportionally. \
